  ...
  return (
    <div className="space-y-5 pb-4 animate-fade-up">
      <Card className="border-caramel-500/20">
        <div className="space-y-3">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-2xs tracking-[0.15em] uppercase text-caramel-400 font-semibold mb-1 select-none">{catLabel(store.category)}</p>
              <h2 className="text-xl font-bold text-ink-50">{store.name}</h2>
              {store.address && <p className="text-sm text-ink-500 mt-1 flex items-center gap-1"><Icon name="map-pin" size={14} />{store.address}</p>}
            </div>
            <Badge variant="accent" size="md">{catLabel(store.category)}</Badge>
          </div>
          {store.description && <p className="text-sm text-ink-200 leading-relaxed pl-3 border-l-2 border-caramel-500/30">{store.description}</p>}
          <div className="flex items-center gap-3 text-2xs text-ink-500 pt-2 border-t border-ink-800/20">
            ...
            {store.hasPrivateRoom && <span className="flex items-center gap-1 text-caramel-400"><Icon name="sparkle" size={12} />有包厢</span>}
          </div>
      ...
      {store.mapLinks?.length > 0 && <Card>
        <div className="flex items-center gap-2 mb-3 select-none"><Icon name="map-pin" size={18} className="text-caramel-400" /><h4 className="font-semibold text-ink-100 text-sm">地图导航</h4></div>
        ...
        <p className="text-2xs text-ink-500 mt-2 text-center select-none">移动端点击跳转地图 APP</p>
      </Card>}
      ...
      {store.ratings?.length > 0 && (
        <div className="space-y-3 stagger">
          <div className="flex items-center gap-2 px-1 select-none"><Icon name="chat" size={18} className="text-caramel-400" /><h4 className="font-semibold text-ink-100 text-sm">大家的评价</h4><span className="text-xs text-ink-500">({store._count?.ratings || store.ratings.length})</span></div>
          {store.ratings.map((rating: any) => (
            <Card key={rating.id} padding="md">
              <div className="space-y-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-caramel-500/20 flex items-center justify-center text-caramel-400 font-semibold text-xs select-none">{(rating.user?.nickname || "?")[0]}</div>
                    <span className="text-sm font-medium text-ink-100">{rating.user?.nickname || "匿名"}</span>
                  </div>
                ...
                <div className="flex items-center gap-1 text-sm">
                  <span className="flex gap-0.5">{[...Array(rating.wantScore)].map((_,j) => <Icon key={j} name="star" size={14} className="text-caramel-400" />)}</span>
                  <span className="text-ink-500 text-xs ml-1">想吃指数</span>
                </div>
                {rating.triedRating !== "NOT_TRIED" && <div className="grid grid-cols-4 gap-1 text-2xs text-ink-300 bg-ink-900/60 rounded-lg p-2">
                ...
                {rating.comment && <p className="text-sm text-ink-200 bg-ink-900/40 rounded-lg p-2.5 leading-relaxed italic">{rating.comment}</p>}
                <p className="text-2xs text-ink-500">{formatRelativeTime(rating.updatedAt || rating.createdAt)}</p>
              </div>
            </Card>
          ))}
        </div>
