...
export function StarRating({ value, onChange, readonly = false, size = "md", label }: StarRatingProps) {
  ...
  return (
    <div>
      {label && <span className="block text-sm font-medium text-paper-300 mb-1.5 select-none">{label}</span>}
        ...
        {[1,2,3,4,5].map(star => (
          <button key={star} type="button" disabled={readonly} onClick={() => onChange?.(star)}
            className={cn("transition-all duration-150 select-none",
              ...
              star <= value ? "text-gold-400" : "text-base-500 hover:text-base-400")}
            aria-label={`${star}星`}>
            <Icon name="star" size={iconSize} />
          </button>
        ))}
        {!readonly && <span className="text-xs text-paper-500 ml-2 mb-0.5 select-none">{value > 0 ? `${value}/5` : "点击评分"}</span>}
      </div>
    </div>
  );
}
