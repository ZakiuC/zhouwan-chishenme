  ...
  return (
    <Card className="border-accent-500/20">
      <form onSubmit={submit} className="space-y-5">
        <div className="flex items-center gap-2 select-none"><Icon name="star" size={18} className="text-gold-400" /><h4 className="font-semibold text-paper-200">{existingRating ? "修改我的评分" : "我的评分"}</h4></div>
        <div className="bg-accent-500/5 rounded-xl p-3 border border-accent-500/10"><StarRating label="想吃指数 *" value={want} onChange={setWant} size="md" /></div>
        <TriedRatingSelector value={tried} onChange={setTried} />
        <div className={`space-y-3 ${!isTried ? "opacity-25 pointer-events-none" : ""}`}>
          <div className="bg-base-800/60 rounded-xl p-3 space-y-2.5">
        ...
        {!isTried && <p className="text-xs text-paper-500 italic">选择吃过评级后解锁完整评分维度</p>}
        <div>
          <label className="block text-sm font-medium text-paper-300 mb-1.5 select-none flex items-center gap-1"><Icon name="chat" size={14} />文字评价（可选）</label>
          <textarea value={comment} onChange={e => setComment(e.target.value)} maxLength={300} rows={2} placeholder="说说你的感受..."
            className="w-full px-4 py-2.5 rounded-xl bg-base-600 border border-base-400/50 text-paper-100 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-accent-400/30 focus:border-accent-400/50 transition-all" />
          <div className="flex justify-between mt-1"><span className="text-2xs text-paper-600">分享真实体验帮助大家选择</span><span className="text-2xs text-paper-600">{comment.length}/300</span></div>
        </div>
