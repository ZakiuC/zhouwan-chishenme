    ...
    <form onSubmit={submit} className="space-y-4 animate-fade-up">
      <Card>
        <div className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-ink-200 mb-1.5 select-none">分类</label><Select value={form.category} onChange={v => up("category", v)} options={STORE_CATEGORIES.map(c => ({ value: c.value, label: c.label }))} placeholder="选择分类" /></div>

          {/* 包厢开关 */}
          <div className="flex items-center justify-between py-1">
            <span className="text-sm font-medium text-ink-200 select-none flex items-center gap-1">
            ...
            <button
              ...
              className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${
                form.hasPrivateRoom ? "bg-caramel-500" : "bg-ink-700"
              }`}
            >
          ...
          <div><label className="block text-sm font-medium text-ink-200 mb-1.5 select-none">简短描述</label><textarea value={form.description} onChange={e => up("description", e.target.value)} maxLength={500} rows={3} className="w-full px-4 py-2.5 rounded-xl bg-ink-800 border border-ink-700/50 text-ink-50 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-caramel-400/30 focus:border-caramel-400/50 transition-all" placeholder="说说这家店的特色..." /><p className="text-2xs text-ink-500 mt-1">{form.description.length}/500</p></div>
      ...
      <Card>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-ink-100 text-sm select-none flex items-center gap-1"><Icon name="map-pin" size={14} />地图链接</h4>
          ...
          {showMapSearch && (
            <div className="bg-ink-900/60 rounded-xl p-3 border border-ink-800/20">
              <MapSearch onSelect={handleMapSelect} onClose={() => setShowMapSearch(false)} />
            </div>
          )}

          {errors.mapLinks && <p className="text-xs text-rust-400">{errors.mapLinks}</p>}
          {form.mapLinks.length === 0 && !showMapSearch && <p className="text-sm text-ink-500 py-2">添加百度/高德/腾讯地图的分享链接，或使用搜索自动填充</p>}
            ...
            return (
              <div key={ml.provider} className="flex items-center gap-2">
                ...
                <div className="flex-1 relative">
                  <input type="url" value={ml.url} onChange={e => upML(i, e.target.value)} placeholder={`粘贴${p?.label}分享链接`} className="w-full h-9 px-3 pr-8 rounded-lg bg-ink-800 border border-ink-700/50 text-ink-50 text-sm focus:outline-none focus:ring-2 focus:ring-caramel-400/30" />
                  <button type="button" onClick={() => rmML(i)} className="absolute right-2 top-1/2 -translate-y-1/2 text-ink-500 hover:text-rust-400"><Icon name="cross" size={14} /></button>
                </div>
              </div>
            );
          })}
